...
def main():
    try:
        ...
        while cont < int.from_bytes(cliente.h3, "big"):
            ...
            # cria CRC
            verifica  = int.from_bytes(cliente.h4,"big") - 1
            dados = cliente.payloads[verifica]
            if verifica == 2:
                dados = cliente.payloads[1]
            crc_1, crc_2 = Crc16.calc(dados).to_bytes(2,byteorder='big')
            cliente.h8 = crc_1.to_bytes(1,byteorder='big')
            cliente.h9 = crc_2.to_bytes(1,byteorder='big')
            # tipo da mensagem 
            tipo = 3
            cliente.h0 = (tipo).to_bytes(1, byteorder="big")
            # Mensagem do tipo Handshake
